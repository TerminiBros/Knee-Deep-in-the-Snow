...
static Sound sfxSnowballHits[3];
static Sound sfxPlayerHurt[2];
...
void LoadAssets(void) { // Loads textures, shaders, audio, fonts, etc.
    ...
    sfxShoot = LoadSound("assets/audio/Shoot.ogg");                                 // Load a sound
    for (size_t i = 0; i < 3; i++) { 
        sfxSnowballHits[i] = LoadSound(TextFormat("assets/audio/SnowballHit%d.ogg", i)); 
        // Lerry Vaud
        SetSoundVolume(sfxSnowballHits[i], 0.3);
    }
    for (size_t i = 0; i < 2; i++) {
        sfxPlayerHurt[i] = LoadSound(TextFormat("assets/audio/Hurt%d.ogg", i)); 
        // Barry Lavid
        // Hurt McGurt
        SetSoundVolume(sfxPlayerHurt[i], 0.5);
    } 
...
// TODO: This is for waves remember this
int lastNum = 10;
int waveCount = 0;
float waveCountDisplayTimer = 0;

void NewWave(void) {
    lastNum = (int)((float)lastNum * 1.5);
    waveCount++;
    waveCountDisplayTimer = 4.0;
...
void SetupEntireGame(void) {
    ...
    {
        float r = 3 + (float)GetRandomValue(0, 10) / 10.0;
...
void UpdateGame(void) {
    ...
    for (size_t i = 0; i < NUM_SPRITES; i++)
    {
        ...
        if (sprites[i].hasAI == true) {
            ...
            // FIXME: HACK TO MAKE THE SNOWMEN FLASH
            if (sprites[i].hasLight) {
                if (sprites[i].light.radius <= 0) {
                    sprites[i].hasLight = false;
                }
                sprites[i].light.radius -= state.deltaTime * 1.5f;
            }
            ...
            case AIS_Wander: {
                ...
                if (ai->stateCooldownEnabled == false) {
                    ai->stateCooldown = GetRandomValue(25,64);
        ...
        if (sprites[i].deathtimerEnabled) {
            if (sprites[i].deathtimer <= 0) {
                sprites[i] = (GameSprite){0};
            }
            sprites[i].deathtimer -= state.deltaTime;
            sprites[i].light.radius -= state.deltaTime;
        }
        ...
        if (sprites[i].type == GST_EnemyBullet) {
            if (CheckCollisionPointCircle( playerPos, (Vector2){sprites[i].x,sprites[i].y}, 0.6f )) {
                ...
                PlaySoundRandom(sfxPlayerHurt, 0.8, 1.0, false);
                PlaySoundRandomPick(sfxSnowballHits, false);
                sprites[i] = (GameSprite){0};    
            }
                        ...
                        if (sprites[ei].enabled && sprites[ei].type == GST_Snowman) {
                            if (Vector2Distance((Vector2){sprites[i].x, sprites[i].y}, (Vector2){sprites[ei].x, sprites[ei].y}) < 3.0) {
                                ...
                                if (GetRandomValue(0,15) == 0) {
                                    sprites[ei].ai.state = AIS_Run;
                                }
                                //int nid = FindFreeID();
                                //SpawnProp(nid, sprites[ei].x, sprites[ei].y, false, (Rectangle){0,0,0,0}, (Vector2){1,1});
                                sprites[ei].hasLight = true;
                                sprites[ei].light.color = sprites[i].light.color;
                                sprites[ei].light.radius = .7f;
                                PlaySoundRandomPick(sfxSnowballHits, true);
                                //sprites[nid].enabled = true;
                                //sprites[nid].deathtimerEnabled = true;
                                //sprites[nid].deathtimer = 0.2;
                            }
                        }
                    }       
                }
    ...
    if (isMouseLocked) {
        ...
        if ( weapons[selectedWeapon].cooldown <= 0) {
            ...
            ||  (weapons[selectedWeapon].holdDown == true  && IsMouseButtonDown(MOUSE_BUTTON_LEFT))
                ...
                if (selectedWeapon == 1) {
                    weapons[selectedWeapon].nextColor = BulbColorPool[GetRandomValue(0,4)];
                }
                PlaySoundRandomPitch(sfxShoot, 0.8, 1.1, true);
            }
        }
    ...
    if (waveCountDisplayTimer > 0) { waveCountDisplayTimer -= state.deltaTime; }
...
void RenderScene(void) {
    ...
    for (size_t i = 0; i < NUM_SPRITES; i++)
    {
        ...
        case GST_PlayerBullet: {
            DrawBillboardRec(cam, texBullets, sprites[i].rect, (Vector3){ sprites[i].x, sprites[i].scale.y / 2 + sprites[i].hover, sprites[i].y }, (Vector2){0.5,0.5}, sprites[i].c);
            break;
        }
...
void DrawGame(void) {
    ...
    float waveTextAlpha = Clamp(waveCountDisplayTimer*2, 0, 1);
    DrawTextCenteredEx(fntReadswell, TextFormat("WAVE:%d", waveCount), (Vector2) {content.x, 18}, 0, 13, 1, Fade(GetColor(0x282285ff), waveCountDisplayTimer));
    DrawTextCenteredEx(fntReadswell, TextFormat("WAVE:%d", waveCount), (Vector2) {content.x, 16}, 0, 13, 1, Fade(GetColor(0xd0d0d0ff), waveCountDisplayTimer));
    ...
    if (fadeTimer > 0) { DrawRectangle(0, 0, content.width, content.height, Fade(bgColor, EaseQuadOut(fadeTimer, 0, 1, 0.4))); fadeTimer -= state.deltaTime; }
}
